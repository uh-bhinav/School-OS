...
class GetMyAttendanceReportSchema(BaseModel):
    ...
    date_range: Optional[str] = Field(
        default=None,
        description="Optional date range (e.g., 'last 30 days', 'this term').",
    )
...
class GetClassAttendanceSheetSchema(BaseModel):
    ...
    date: datetime.date = Field(
        ...
        description="The date for the attendance sheet (YYYY-MM-DD). Defaults to today.",
    )
...
class TakeClassAttendanceSchema(BaseModel):
    ...
    date: datetime.date = Field(
        default_factory=date.today,
        description="The date of the attendance (YYYY-MM-DD). Defaults to today.",
    )
...
class GetLowAttendanceReportSchema(BaseModel):
    ...
    end_date: date = Field(
        default_factory=date.today,
        description="The end date for the report period (YYYY-MM-DD). Defaults to today.",
    )
...
__all__ = [
    "GetMyAttendanceReportSchema",
    "GetClassAttendanceSheetSchema",
    "TakeClassAttendanceSchema",
    "GetAllAbsenteesTodaySchema",
    "GetLowAttendanceReportSchema",
]
