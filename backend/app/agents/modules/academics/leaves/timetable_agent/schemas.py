...
class GetMyTimetableSchema(BaseModel):
    ...
    timetable_date: Optional[date] = Field(
        default=None,
        description="Optional date (YYYY-MM-DD). If not provided, today's schedule is assumed.",
    )
    date_range: Optional[str] = Field(
        default=None,
        description="Optional range, e.g., 'this week' or 'next week'. The service will interpret this.",
    )
...
class GetClassScheduleSchema(BaseModel):
    ...
    class_name: str = Field(
        ...,
        description="The name of the class (e.g., '10A' or 'Grade 10 - Section A').",
    )
    day: Optional[date] = Field(
        default=None,
        description="The specific date (YYYY-MM-DD) for the schedule. Defaults to today if not provided.",
    )
...
class GenerateTimetableSchema(BaseModel):
    ...
    class_name: str = Field(
        ...,
        description="The name of the class for which to auto-generate a new timetable.",
    )
...
class FindFreeSlotSchema(BaseModel):
    ...
    day: Optional[date] = Field(
        default=None,
        description="Optional date (YYYY-MM-DD) to search. Defaults to today.",
    )
