...
from app.agents.modules.academics.leaves.class_agent.prompts import (
    SYSTEM_PROMPT,
)
from app.agents.modules.academics.leaves.class_agent.tools import (
    class_agent_tools,
)
...
class ClassAgent(BaseAgent):
    """
    A specialized leaf agent for managing Classes.
    ...
    - Creating, searching, and updating classes
    ...
    def invoke(self, query: str, conversation_history: Optional[list] = None) -> dict[str, Any]:
        ...
        """
        try:
            logger.info(f"ClassAgent invoked with query: '{query[:100]}...'")
            ...
            logger.info("ClassAgent invocation completed successfully")
            ...
            logger.error(f"ClassAgent invocation failed: {e}", exc_info=True)
            return {
                "response": "I apologize, but I encountered an error while processing " "your request regarding classes. Please try again.",
                "messages": [],
                "success": False,
                "error": str(e),
            }
