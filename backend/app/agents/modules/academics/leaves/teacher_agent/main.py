...
from app.agents.modules.academics.leaves.teacher_agent.prompts import (
    SYSTEM_PROMPT,
)
from app.agents.modules.academics.leaves.teacher_agent.tools import (
    teacher_agent_tools,
)
...
class TeacherAgent(BaseAgent):
    """
    A specialized leaf agent for managing Teacher Profiles.
    ...
    - Searching, listing, and updating teacher profiles
    ...
    def __init__(self, llm_tier: str = "medium"):
        """
        Initializes the TeacherAgent with its specific tools and LLM tier.
        ...
        super().__init__(tools=teacher_agent_tools, llm_tier=llm_tier)
        logger.info(f"TeacherAgent initialized with {len(teacher_agent_tools)} tools " f"and '{llm_tier}' tier LLM")
    ...
    async def ainvoke(self, query: str, conversation_history: Optional[list] = None) -> dict[str, Any]:
        ...
        """
        try:
            logger.info(f"TeacherAgent invoked with query: '{query[:100]}...'")
            ...
            logger.info("TeacherAgent invocation completed successfully")
            ...
            logger.error(f"TeacherAgent invocation failed: {e}", exc_info=True)
            return {
                "response": "I apologize, but I encountered an error while processing " "your request regarding teacher profiles. Please try again.",
                "messages": [],
                "success": False,
                "error": str(e),
            }
...
# Create a single, reusable instance of the TeacherAgent.
# This instance will be imported by the API layer and parent routers.
teacher_agent_instance = TeacherAgent()


# For backward compatibility / consistency with your AttendanceAgent
teacher_agent_app = teacher_agent_instance


# Convenience function for direct invocation
def invoke_teacher_agent(query: str) -> str:
    """
    Convenience function to invoke the TeacherAgent and return just the response string.
    """
    result = teacher_agent_instance.ainvoke(query)
