...
from app.agents.modules.academics.leaves.exam_agent.prompts import (
    SYSTEM_PROMPT,
)
from app.agents.modules.academics.leaves.exam_agent.tools import (
    exam_agent_tools,
)
...
class ExamAgent(BaseAgent):
    """
    A specialized leaf agent for managing Exam schedules.
    ...
    - Creating, searching, updating, and deleting exam schedules.

    It uses a 'medium' tier LLM for balanced performance and capability.
    """
    ...
    def __init__(self, llm_tier: str = "medium"):
        ...
        logger.info(f"ExamAgent initialized with {len(exam_agent_tools)} tools " f"and '{llm_tier}' tier LLM")
    ...
    async def ainvoke(self, query: str, conversation_history: Optional[list] = None) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "response": "I apologize, but I encountered an error while processing " "your request regarding exam schedules. Please try again.",
                "messages": [],
                "success": False,
                "error": str(e),
            }
...
async def invoke_exam_agent(query: str) -> str:
    """
