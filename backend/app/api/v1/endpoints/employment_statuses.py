...
async def get_status_by_id(status_id: int, db: AsyncSession = Depends(get_db)):
    ...
    status = await employment_status_service.get_status_by_id(
        db=db, status_id=status_id
    )
    if not status:
        raise HTTPException(
...
async def update_existing_status(
    ...
    *,
    ...
    db_status = await employment_status_service.get_status_by_id(
        db=db, status_id=status_id
    )
    if not db_status:
        raise HTTPException(
    ...
    return await employment_status_service.update_status(
        db=db, db_obj=db_status, status_in=status_in
    )
