...
async def create_fee_component(component_in: FeeComponentCreate, service: FeeStructureService = Depends(get_fee_structure_service), current_user: Profile = Depends(get_current_user_profile)):
    ...
    if hasattr(component_in, "school_id") and component_in.school_id != current_user.school_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin cannot create fee components for other schools.")
...
async def get_fee_components_for_school(school_id: int, service: FeeStructureService = Depends(get_fee_structure_service), current_user: Profile = Depends(get_current_user_profile)):
    """Get all fee components for a specific school."""
    if school_id != current_user.school_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin can only view fee components for their own school.")
...
async def create_fee_template(
    ...
    if hasattr(template_in, "school_id") and template_in.school_id != current_user.school_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin cannot create fee templates for other schools.")
...
@router.get("/fee-components/{component_id}", response_model=FeeComponentOut, dependencies=[Depends(require_role("Admin"))])
async def get_fee_component_by_id(component_id: int, service: FeeStructureService = Depends(get_fee_structure_service), current_user: Profile = Depends(get_current_user_profile)):
...
@router.put("/fee-components/{component_id}", response_model=FeeComponentOut, dependencies=[Depends(require_role("Admin"))])
async def update_fee_component_by_id(
    component_id: int, component_in: FeeComponentUpdate, service: FeeStructureService = Depends(get_fee_structure_service), current_user: Profile = Depends(get_current_user_profile)  # Assumes FeeComponentUpdate schema exists
):
...
@router.delete("/fee-components/{component_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_role("Admin"))])
async def delete_fee_component_by_id(component_id: int, service: FeeStructureService = Depends(get_fee_structure_service), current_user: Profile = Depends(get_current_user_profile)):
...
async def get_fee_templates_for_school(
    ...
    if school_id != current_user.school_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin can only view fee templates for their own school.")
...
async def assign_template_to_class(assignment_in: AssignTemplateToClassSchema, service: FeeStructureService = Depends(get_fee_structure_service), current_user: Profile = Depends(get_current_user_profile), db: AsyncSession = Depends(get_db)):
    """
    ...
    if target_class.school_id != current_user.school_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin cannot assign templates to classes in other schools.")
    ...
    if not target_template or target_template.school_id != current_user.school_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Fee template not found or does not belong to this school.")  # Or 404/403
