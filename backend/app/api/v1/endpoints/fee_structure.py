...
from app.schemas.fee_component_schema import (
    FeeComponentCreate,
    FeeComponentOut,
    FeeComponentUpdate,
)
...
@router.get(
    "/fee-components/school/{school_id}",
    response_model=list[FeeComponentOut],
...
async def update_fee_component_by_id(
    component_id: int,
    component_in: FeeComponentUpdate,
    service: FeeStructureService = Depends(get_fee_structure_service),
    current_user: Profile = Depends(get_current_user_profile),  # Assumes FeeComponentUpdate schema exists
):
    ...
    if component.school_id != current_user.school_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Admin cannot update components for other schools.",
        )
    # Prevent changing school_id if present in update schema
    if hasattr(component_in, "school_id") and component_in.school_id != current_user.school_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot change component's school.",
        )
...
async def delete_fee_component_by_id(
    ...
    if component.school_id != current_user.school_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Admin cannot delete components for other schools.",
        )
    ...
    if is_used:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot delete fee component as it is used in active fee templates.",
        )
