...
from app.schemas.student_fee_discount_schema import (
    StudentFeeDiscountCreate,
    StudentFeeDiscountOut,
)
...
async def create_discount_template(
    ...
    if hasattr(discount_in, "school_id") and discount_in.school_id != current_user.school_id:
        raise HTTPException(
            ...
            detail="Admin cannot create discount templates for other schools.",
        )
...
async def get_discount_templates_for_school(
    ...
    if school_id != current_user.school_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only view discount templates for your own school.",
        )
    return await service.get_discounts_by_school(school_id=school_id)


@router.post(
    "/discounts/apply-to-student",
    response_model=StudentFeeDiscountOut,
    status_code=201,
    dependencies=[Depends(require_role("Admin"))],
)
async def apply_discount_to_student(
    ...
    if target_student.profile.school_id != current_user.school_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Admin cannot apply discounts to students in other schools.",
        )

    return await service.apply_discount_to_student(
        application_data=application_in,
