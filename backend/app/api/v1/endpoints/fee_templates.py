...
@router.get(
    "/fee-templates/{template_id}",
    response_model=FeeTemplateOut,
    dependencies=[Depends(require_role("Admin"))],
)
async def get_fee_template_by_id(
    template_id: int,
    service: FeeStructureService = Depends(get_fee_template_service),
...
async def update_fee_template_by_id(
    template_id: int,
    template_in: FeeTemplateUpdate,
    service: FeeStructureService = Depends(get_fee_template_service),
    current_user: Profile = Depends(get_current_user_profile),  # Assumes FeeTemplateUpdate schema exists
):
    ...
    if template.school_id != current_user.school_id:
        raise HTTPException(
            ...
            detail="Admin cannot update templates for other schools.",
        )
    ...
    if hasattr(template_in, "school_id") and template_in.school_id is not None and template_in.school_id != current_user.school_id:
        raise HTTPException(
            ...
            detail="Cannot change template's school.",
        )
...
async def delete_fee_template_by_id(
    ...
    if template.school_id != current_user.school_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Admin cannot delete templates for other schools.",
        )
