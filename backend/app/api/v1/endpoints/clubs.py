...
@router.get("/agent/student-clubs/{student_id}", response_model=dict, summary="[AGENT] Get clubs for a student by ID")
async def agent_get_student_clubs(
    ...
    try:
        ...
        if not memberships:
            return {"success": True, "student_id": student_id, "count": 0, "clubs": [], "message": f"Student {student_id} is not a member of any clubs."}
        ...
        return {"success": True, "student_id": student_id, "count": len(clubs_data), "data": clubs_data}
...
@router.get("/agent/club-members-list/{club_name}", response_model=dict, summary="[AGENT] Get club members with student_id")
async def agent_get_club_members_list(
    ...
    try:
        ...
        if not club:
            return {"success": False, "error": f"Club '{club_name}' not found", "club_name": club_name, "count": 0, "members": []}

        # 2. Get all memberships with student data
        memberships = await service.get_club_memberships_by_club(club.id, school_id)

        if not memberships:
            return {"success": True, "club_name": club_name, "count": 0, "members": [], "message": f"No active members in {club_name}"}
        ...
        return {"success": True, "club_name": club_name, "count": len(members_data), "members": members_data}
...
@router.post("/agent/remove-member", status_code=status.HTTP_200_OK, dependencies=[Depends(RoleChecker(admin_or_teacher_roles))], summary="[AGENT] Remove student from club using names")
async def agent_remove_member(member_in: AgentAddMember, db: AsyncSession = Depends(get_db_session), current_user: Profile = Depends(get_current_user)):
    """
    ...
    try:
        ...
        return {"success": True, "message": f"Student {member_in.student_name} has been removed from {member_in.club_name}", "club_id": club.id, "student_id": student.student_id}

    except HTTPException:
        raise
    except Exception as e:
        logger.exception(f"Error removing student from club: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error removing student from club: {str(e)}")
