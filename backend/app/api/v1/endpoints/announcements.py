...
async def update_package_header_by_id(
    ...
    """
    # Fetch the object directly by PK to allow updates even if it was inactive
    package = await db.get(ProductPackage, package_id)
    if not package:
...
async def delete_package_by_id(package_id: int, db: AsyncSession = Depends(get_db)):
    ...
    CRITICAL FIX: Uses db.get to retrieve
    the object regardless of 'is_active' status
    before performing the soft delete.
    """
    # Fetch the object directly by PK to
    #  allow soft deletion or re-deletion
    package = await db.get(ProductPackage, package_id)
    if not package:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product Package not found")

    if not package.is_active:
        # If it's already inactive, the DELETE
        #  operation is implicitly successful (Idempotency).
        return None
