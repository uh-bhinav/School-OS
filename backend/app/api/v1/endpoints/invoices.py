...
async def get_student_invoices(
    ...
    is_parent_contact_stmt = (
        select(StudentContact)
        .where(StudentContact.student_id == student_id)
        .where(StudentContact.profile_user_id == current_user.user_id)
    )
...
async def get_invoice(
    ...
    is_parent_contact_stmt = (
        select(StudentContact)
        .where(StudentContact.student_id == invoice.student_id)
        .where(StudentContact.profile_user_id == current_user.user_id)
    )
...
@router.get(
    ...
    dependencies=[Depends(require_role('Admin'))] # Protect with Admin role
)
async def list_all_school_invoices_admin(
    db: AsyncSession = Depends(get_db),
    current_user: Profile = Depends(get_current_user_profile) # Get authenticated admin user
):
    """
    ...
    invoices = await invoice_service.get_all_invoices_for_school(
        db=db, 
