...
class Club(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "max_members IS NULL OR current_member_count <= max_members",
            name="chk_club_member_capacity",
        ),
        CheckConstraint(
            "registration_end_date IS NULL OR registration_end_date > registration_start_date",
            name="chk_club_registration_dates",
        ),
    )
