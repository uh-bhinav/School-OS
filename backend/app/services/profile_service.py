...
async def get_all_profiles_for_school(
    ...
    """
    stmt = select(Profile).where(Profile.school_id == school_id).options(selectinload(Profile.roles).selectinload(UserRole.role_definition)).order_by(Profile.last_name)
