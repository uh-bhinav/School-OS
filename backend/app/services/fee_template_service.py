...
class FeeTemplateService:
    ...
    async def get_template(db: AsyncSession, *, template_id: int) -> Optional[FeeTemplate]:
        ...
        """
        stmt = select(FeeTemplate).where(FeeTemplate.id == template_id).options(selectinload(FeeTemplate.fee_terms))  # Eager load terms
    ...
    async def get_all_templates(db: AsyncSession, school_id: int) -> list[FeeTemplate]:
        ...
        """
        stmt = select(FeeTemplate).where(FeeTemplate.school_id == school_id).options(selectinload(FeeTemplate.fee_terms)).order_by(FeeTemplate.start_date.desc(), FeeTemplate.name)  # Eager load terms  # Added name ordering
        result = await db.execute(stmt)
