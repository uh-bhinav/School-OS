...
async def get_stream_by_id(db: AsyncSession, stream_id: int) -> Optional[Stream]:
    ...
    """
    stmt = select(Stream).where(Stream.id == stream_id).options(selectinload(Stream.subjects))
    result = await db.execute(stmt)
    return result.scalars().first()


async def get_all_streams_for_school(db: AsyncSession, *, school_id: int) -> list[Stream]:
    """
    Retrieves a list of all available academic streams for a given school.
    """
    stmt = select(Stream).where(Stream.school_id == school_id, Stream.is_active.is_(True)).options(selectinload(Stream.subjects)).order_by(Stream.name)
    result = await db.execute(stmt)
    return list(result.scalars().all())


async def update_stream(db: AsyncSession, *, db_stream: Stream, stream_in: StreamUpdate) -> Stream:
    """
...
async def assign_subjects_to_stream(db: AsyncSession, *, db_stream: Stream, subject_ids: list[int]) -> Stream:
    """
