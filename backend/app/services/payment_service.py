...
class PaymentService:
    ...
    async def initiate_payment(self, *, request_data: PaymentInitiateRequest, user_id: str) -> dict:
        ...
        # 2. Create our internal 'pending' payment record [cite: 547-549, 1148]
        ...
        # 3. Retrieve and decrypt school's Razorpay credentials [cite: 550-551, 1150]
        ...
        # 4. Call Razorpay's Orders API [cite: 552-553, 1151]
        try:
        ...
        # 5. Update our payment record with the gateway order ID [cite: 556, 1154]
        new_payment.gateway_order_id = razorpay_order["id"]
        await self.db.commit()
        await self.db.refresh(new_payment)

        # 6. Return data required by the frontend [cite: 557, 1155]
    ...
    async def verify_payment(self, *, verification_data: PaymentVerificationRequest) -> Payment:
        ...
        # 2. Retrieve and decrypt the school's Razorpay secret [cite: 1310]
        ...
        # 3. Perform the cryptographic signature verification [cite: 1310, 1908]
        try:
        ...
        # 4. If verification succeeds, update our records atomically [cite: 1312-1313, 1909-1910]
        ...
        if payment.invoice_id:
            ...
            if invoice:
                ...
                # and then update status based on its result.
                invoice.payment_status = "paid"  # Simplified for now
    ...
    async def handle_webhook_event(self, *, payload: dict, signature: str) -> None:
        ...
        if event_type == "payment.captured":
            try:
                ...
                # 2. Signature Verification [cite: 809, 915-917]
                ...
                # 3. Process the event if signature is valid
                # This is our backup logic. If the 'verify' endpoint failed, this will catch it.
                if payment.status != "captured":
                    payment.status = "captured"
                    payment.gateway_payment_id = payment_entity.get("id")
                    # ... add logic to update Invoice/Order status here as well
