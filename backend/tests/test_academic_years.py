...
async def test_set_active_academic_year_deactivates_others(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    year_A_id = (await test_client.post("/api/v1/academic-years/", json=year_A_payload)).json()["id"]
    year_B_id = (await test_client.post("/api/v1/academic-years/", json=year_B_payload)).json()["id"]

    await test_client.put(f"/api/v1/academic-years/{SCHOOL_ID}/set-active/{year_A_id}")
    ...
    """response_A = await test_client.get(f"/api/v1/academic-years/{year_A_id}")
