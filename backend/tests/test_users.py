...
# --- Integration Tests for User Invitation (/api/v1/users/invite) ---
...
async def test_invite_new_user_as_admin_success(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    with patch(
        ...
        response = await test_client.post("/api/v1/users/invite", json=VALID_INVITE_PAYLOAD)
...
async def test_invite_user_with_missing_fields_fails(
    ...
    response = await test_client.post("/api/v1/users/invite", json=invalid_payload)
