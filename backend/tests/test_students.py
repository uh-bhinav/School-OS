...
async def test_search_student_by_name(test_client: AsyncClient, mock_admin_profile: Profile, db_session: AsyncSession):
    ...
    with patch(
        ...
        response = await test_client.get(f"/api/v1/students/search?name={unique_name}")
        print(response.text)
...
async def test_student_soft_deletion(test_client: AsyncClient, mock_admin_profile: Profile, db_session: AsyncSession):
    ...
    with patch(
        ...
        delete_response = await test_client.delete("/api/v1/students/101")
    ...
    with patch(
        ...
        delete_non_existent_response = await test_client.delete("/api/v1/students/99999")
...
async def test_bulk_promote_students(test_client: AsyncClient, mock_admin_profile: Profile, db_session: AsyncSession):
    ...
    with patch(
        ...
        response = await test_client.post("/api/v1/students/promote", json=promotion_payload)
