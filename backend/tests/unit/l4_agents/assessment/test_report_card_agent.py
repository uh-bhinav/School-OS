...
async def test_happy_path_get_student_report_card(mock_report_card_llm_invoke, mock_report_card_http_client):
    ...
    mock_report_card_llm_invoke.side_effect = [
        AIMessage(content="", tool_calls=[tool_call]),
        AIMessage(content="Here is the report card: Physics - 85, Math - 90."),
    ]
    ...
    mock_report_card_http_client.get.assert_called_once_with(
        f"/report-cards/student/{tool_args['student_id']}",
        params={"academic_year_id": tool_args["academic_year_id"]},
    )
