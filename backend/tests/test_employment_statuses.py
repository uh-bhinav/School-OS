...
async def test_create_employment_status_as_admin(test_client: AsyncClient, mock_admin_profile: Profile):
    ...
    create_response = await test_client.post("/v1/employment-statuses/", json=create_payload)
...
async def test_get_employment_status_by_id_as_admin(
    ...
    app.dependency_overrides[get_current_user_profile] = lambda: mock_admin_profile
    get_response = await test_client.get(f"/v1/employment-statuses/{created_employment_status.status_id}")
...
async def test_get_all_statuses_for_school_as_admin(
    ...
    app.dependency_overrides[get_current_user_profile] = lambda: mock_admin_profile
    get_all_response = await test_client.get(f"/v1/employment-statuses/{SCHOOL_ID}/all")
...
async def test_update_employment_status_as_admin(
    ...
    update_response = await test_client.put(
        f"/v1/employment-statuses/{created_employment_status.status_id}",
        json=update_payload,
    )
...
async def test_delete_employment_status_as_admin(
    ...
    app.dependency_overrides[get_current_user_profile] = lambda: mock_admin_profile
    delete_response = await test_client.delete(f"/v1/employment-statuses/{created_employment_status.status_id}")
    assert delete_response.status_code == status.HTTP_204_NO_CONTENT

    # Verify the status is now deleted
    verify_response = await test_client.get(f"/v1/employment-statuses/{created_employment_status.status_id}")
...
async def test_create_status_as_non_admin_fails(test_client: AsyncClient, mock_teacher_profile: Profile):
    ...
    response = await test_client.post("/v1/employment-statuses/", json=create_payload)
...
async def test_update_delete_non_existent_status_fails(test_client: AsyncClient, mock_admin_profile: Profile):
    ...
    # Test UPDATE
    update_response = await test_client.put(f"/v1/employment-statuses/{non_existent_id}", json={"status_name": "Wont Work"})
    assert update_response.status_code == status.HTTP_404_NOT_FOUND

    # Test DELETE
    delete_response = await test_client.delete(f"/v1/employment-statuses/{non_existent_id}")
