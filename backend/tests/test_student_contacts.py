...
async def test_create_student_contact_as_admin(test_client: AsyncClient, mock_admin_profile: Profile, mock_parent_profile: Profile):
    ...
    create_response = await test_client.post("/v1/student-contacts/", json=create_payload)
...
async def test_get_student_contact_by_id_as_admin(
    ...
    app.dependency_overrides[get_current_user_profile] = lambda: mock_admin_profile
    get_response = await test_client.get(f"/v1/student-contacts/{created_student_contact.id}")
...
async def test_get_all_contacts_for_student_as_admin(
    ...
    app.dependency_overrides[get_current_user_profile] = lambda: mock_admin_profile
    get_all_response = await test_client.get(f"/v1/student-contacts/student/{EXISTING_STUDENT_ID}")
...
async def test_update_student_contact_as_admin(
    ...
    update_response = await test_client.put(f"/v1/student-contacts/{created_student_contact.id}", json=update_payload)
...
async def test_delete_student_contact_as_admin(
    ...
    app.dependency_overrides[get_current_user_profile] = lambda: mock_admin_profile
    delete_response = await test_client.delete(f"/v1/student-contacts/{created_student_contact.id}")
    assert delete_response.status_code == status.HTTP_204_NO_CONTENT

    # Verify the contact is now inactive
    verify_response = await test_client.get(f"/v1/student-contacts/{created_student_contact.id}")
...
async def test_create_contact_as_non_admin_fails(
    ...
    response = await test_client.post("/v1/student-contacts/", json=create_payload)
...
async def test_update_delete_non_existent_contact_fails(test_client: AsyncClient, mock_admin_profile: Profile):
    ...
    # Test UPDATE
    update_response = await test_client.put(f"/v1/student-contacts/{non_existent_id}", json={"phone": "0000000000"})
    assert update_response.status_code == status.HTTP_404_NOT_FOUND

    # Test DELETE
    delete_response = await test_client.delete(f"/v1/student-contacts/{non_existent_id}")
