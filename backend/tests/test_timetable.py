...
async def test_timetable_crud_lifecycle_as_admin(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    year_resp = await test_client.post("/v1/academic-years/", json=year_payload)
    ...
    class_resp = await test_client.post("/v1/classes/", json=class_payload)
    ...
    subject_resp = await test_client.post("/v1/subjects/", json=subject_payload)
    ...
    period_resp = await test_client.post("/v1/periods/", json=period_payload)
    ...
    create_response = await test_client.post("/v1/timetable/", json=create_payload)
    ...
    # DELETE
    delete_response = await test_client.delete(f"/v1/timetable/{entry_id}")
...
async def test_get_timetable_for_class_as_teacher(test_client: AsyncClient, db_session: AsyncSession):
    ...
    response = await test_client.get(f"/v1/timetable/classes/{EXISTING_CLASS_ID}")
...
async def test_create_timetable_entry_as_teacher_fails(test_client: AsyncClient, db_session: AsyncSession):
    ...
    response = await test_client.post("/v1/timetable/", json=payload)
...
async def test_get_timetable_for_teacher_as_self(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    response = await test_client.get(f"/v1/timetable/teachers/{TARGET_TEACHER_ID}")
...
async def test_get_teacher_timetable_as_student_fails(test_client: AsyncClient, db_session: AsyncSession):
    ...
    response = await test_client.get("/v1/timetable/teachers/11")
...
async def test_get_schedule_for_day_all_targets(
    ...
    await test_client.post("/v1/timetable/", json=entry_payload)
...
async def test_update_or_delete_non_existent_entry_fails(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    # Test DELETE
    delete_response = await test_client.delete(f"/v1/timetable/{non_existent_id}")
