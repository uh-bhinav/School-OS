...
async def test_period_crud_lifecycle_as_admin(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    create_response = await test_client.post("/api/v1/periods/", json=create_payload)
    ...
    # 2. READ (Get One)
    get_response = await test_client.get(f"/api/v1/periods/{period_id}")
    assert get_response.status_code == status.HTTP_200_OK
    assert get_response.json()["period_name"] == unique_name

    # 3. UPDATE
    update_payload = {"end_time": "09:50:00"}
    update_response = await test_client.put(f"/api/v1/periods/{period_id}", json=update_payload)
    assert update_response.status_code == status.HTTP_200_OK
    assert update_response.json()["end_time"] == "09:50:00"

    # 4. DELETE
    delete_response = await test_client.delete(f"/api/v1/periods/{period_id}")
    assert delete_response.status_code == status.HTTP_204_NO_CONTENT

    # 5. VERIFY DELETE (by trying to get it again)
    verify_response = await test_client.get(f"/api/v1/periods/{period_id}")
...
async def test_create_period_as_teacher_fails(test_client: AsyncClient, db_session: AsyncSession):
    ...
    response = await test_client.post("/api/v1/periods/", json=payload)
...
async def test_update_non_existent_period_fails(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    response = await test_client.put(f"/api/v1/periods/{non_existent_id}", json=update_payload)
...
async def test_delete_non_existent_period_fails(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    response = await test_client.delete(f"/api/v1/periods/{non_existent_id}")
...
async def test_get_all_periods_as_admin(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    await test_client.post("/api/v1/periods/", json=payload)

    # Now, call the 'get all' endpoint
    response = await test_client.get(f"/api/v1/periods/school/{SCHOOL_ID}/all")
...
async def test_get_periods_for_class_as_teacher(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    create_class_resp = await test_client.post("/api/v1/classes/", json=class_payload)
    ...
    app.dependency_overrides[get_current_user_profile] = lambda: mock_teacher_profile
    response = await test_client.get(f"/api/v1/periods/class/{class_id}/periods")
...
async def test_get_recess_periods_as_admin(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    await test_client.post("/api/v1/periods/", json=recess_payload)

    # Act: Call the new endpoint
    response = await test_client.get(f"/api/v1/periods/school/{SCHOOL_ID}/recess")
...
async def test_get_periods_for_non_existent_class(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    response = await test_client.get(f"/api/v1/periods/class/{non_existent_class_id}/periods")
...
async def test_get_recess_for_unauthorized_school_fails(test_client: AsyncClient, db_session: AsyncSession, mock_admin_profile: Profile):
    ...
    response = await test_client.get(f"/api/v1/periods/school/{UNAUTHORIZED_SCHOOL_ID}/recess")
