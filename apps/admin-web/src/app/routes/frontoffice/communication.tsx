...
export default function FrontOfficeCommunication() {
  ...
  const handleSendBroadcast = async (e: React.FormEvent) => {
    ...
    if (!broadcastMessage || !targetAudience) { // ✅ Changed from targetGroup
        alert('⚠️ Please select target audience and enter message');
        return;
      }

    if (confirm(`Send ${priority.toUpperCase()} ${broadcastChannel.toUpperCase()} broadcast to ${targetAudience.replace('_', ' ')}?`)) {
        try {
            await sendBroadcastMutation.mutateAsync({
            message: broadcastMessage,
            channel: broadcastChannel,
            target_audience: targetAudience, // ✅ Changed from target_group
            priority: priority,
        });
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <form onSubmit={handleSendBroadcast}>
            <div className="mb-4">
              ...
              <select
                value={targetAudience} // ✅ Changed from targetGroup
                onChange={(e) => setTargetAudience(e.target.value)} // ✅ Changed
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500"
              >
                <option value="">Select target...</option>
                <option value="all_staff">All Staff (80 people)</option>
                <option value="all_teachers">All Teachers (45 people)</option>
                <option value="all_parents">All Parents (450 people)</option>
                <option value="specific_class">Specific Class</option>
                <option value="transport_staff">Transport Staff (15 people)</option>
                <option value="cafeteria_staff">Cafeteria Staff (8 people)</option>
              </select>
            </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-600 capitalize">{call.call_reason.replace('_', ' ')}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {(broadcast.priority || 'normal').toUpperCase()}
                      </span>
