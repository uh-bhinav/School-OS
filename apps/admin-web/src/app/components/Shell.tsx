...
export function Shell() {
  ...
  const [frontOfficeOpen, setFrontOfficeOpen] = useState(true); // ✅ ADD THIS
  const { clear, role} = useAuthStore();
  const navigate = useNavigate();
  const location = useLocation();
  const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({ academics: true, frontoffice: true});
